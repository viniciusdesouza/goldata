...
export default function PartidasCompartilhadas() {
  const { compartilhados } = useCompartilhados();
  ...
  useEffect(() => {
    ...
    const fetchMatches = () => {
      ...
      fetch(`/api/football/partidas-futebol/fixturesByIds?ids=${compartilhados.join(",")}`)
        .then(res => res.json())
        .then(data => {
          if (run) setMatches(Array.isArray(data.response) ? data.response : []);
        })
        .finally(() => { if (run) setLoading(false); });
    };

    fetchMatches();
    interval = setInterval(fetchMatches, POLLING_INTERVAL);
  ...
  if (loading) {
    return (
      <div className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-2xl mt-2 p-4 w-full max-w-[850px] mx-auto">
        <div className="text-cyan-800 dark:text-cyan-200 text-center py-10 font-semibold">
          Carregando partidas compartilhadas...
        </div>
      </div>
    );
  }

  if (compartilhados.length === 0 || matches.length === 0) {
    return (
      <div className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-2xl mt-2 p-4 w-full max-w-[850px] mx-auto">
        <div className="text-cyan-800 dark:text-cyan-200 text-center py-10 font-semibold">
          Nenhuma partida compartilhada por você.
        </div>
      </div>
    );
  }
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-3 mt-1 px-1">
              ...
              <span className="font-semibold text-cyan-900 dark:text-cyan-200 text-lg">{country}</span>
            </div>
