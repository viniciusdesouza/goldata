"use client";
import { useCompartilhadosRecebidos } from "./CompartilhadosRecebidosContext";
...
export default function PartidasRecebidas() {
  const { compartilhadosRecebidos } = useCompartilhadosRecebidos();
  const [matches, setMatches] = useState<Match[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!compartilhadosRecebidos.length) {
      setMatches([]);
      setLoading(false);
      return;
    }
    ...
    fetch(`/api/football/partidas-futebol/fixturesByIds?ids=${compartilhadosRecebidos.join(",")}`)
      .then(res => res.json())
    ...
    return (
      <div className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-2xl mt-2 p-4 w-full max-w-[850px] mx-auto">
        <div className="text-cyan-800 dark:text-cyan-200 text-center py-10 font-semibold">
          Carregando partidas recebidas...
        </div>
      </div>
    );
  }

  if (compartilhadosRecebidos.length === 0 || matches.length === 0) {
    return (
      <div className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-2xl mt-2 p-4 w-full max-w-[850px] mx-auto">
        <div className="text-cyan-800 dark:text-cyan-200 text-center py-10 font-semibold">
          Nenhuma partida compartilhada com você.
        </div>
      </div>
    );
  }
